let listHoa = [
    {
        mahoa: 1,
        maloai: 1,
        tenhoa: "Đón xuân",
        dongia: 50000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa cúc các màu: trắng, vàng, cam",
    },

    {
        mahoa: 2,
        maloai: 1,
        tenhoa: "Hồn nhiên",
        dongia: 60000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa cúc vàng, cam. lá măng",
    },

    {
        mahoa: 3,
        maloai: 1,
        tenhoa: "Tím thuỷ chung",
        dongia: 45000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa cúc tím",
    },

    {
        mahoa: 4,
        maloai: 1,
        tenhoa: "Nét duyên tím hoa cà",
        dongia: 40000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa cúc màu tím nhạt",
    },

    {
        mahoa: 5,
        maloai: 1,
        tenhoa: "Cùng khoe sắc",
        dongia: 70000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa cúc các màu: trắng, vàng, cam",
    },

    {
        mahoa: 6,
        maloai: 1,
        tenhoa: "Trắng thơ ngây",
        dongia: 65000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa cúc trắng",
    },

    {
        mahoa: 7,
        maloai: 2,
        tenhoa: "Dây tơ hồng",
        dongia: 250000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng màu hồng đậm, hoa hồng xanh, các loại lá măng",
    },

    {
        mahoa: 8,
        maloai: 2,
        tenhoa: "Cầu thuỷ tinh",
        dongia: 220000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng và hoa thuỷ tiên trắng",
    },

    {
        mahoa: 9,
        maloai: 2,
        tenhoa: "Duyên thầm",
        dongia: 260000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa cúc trắng, baby, lá măng",
    },

    {
        mahoa: 10,
        maloai: 2,
        tenhoa: "Ðâm chồi nảy lộc",
        dongia: 180000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng trắng và các loại lá măng",
    },

    {
        mahoa: 11,
        maloai: 2,
        tenhoa: "Hoà quyện",
        dongia: 270000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng trắng, lá thuỷ tùng",
    },

    {
        mahoa: 12,
        maloai: 2,
        tenhoa: "Nồng nàn",
        dongia: 210000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng đỏ, lá thuỷ tùng, lá măng",
    },

    {
        mahoa: 13,
        maloai: 3,
        tenhoa: "Together",
        dongia: 120000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hồng xác pháo, cúc tím",
    },

    {
        mahoa: 14,
        maloai: 3,
        tenhoa: "Long trip",
        dongia: 85000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng đỏ, lá kim thuỷ tùng",
    },

    {
        mahoa: 15,
        maloai: 3,
        tenhoa: "Beautiful life",
        dongia: 100000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng đỏ, lá măng, lá đệm",
    },

    {
        mahoa: 16,
        maloai: 3,
        tenhoa: "Morning Sun",
        dongia: 75000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng vàng",
    },

    {
        mahoa: 17,
        maloai: 3,
        tenhoa: "Pretty Bloom",
        dongia: 65000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng trắng và lá thông",
    },

    {
        mahoa: 18,
        maloai: 3,
        tenhoa: "Red Rose",
        dongia: 45000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng đỏ và lá măng",
    },

    {
        mahoa: 19,
        maloai: 4,
        tenhoa: "Vấn vuơng",
        dongia: 150000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng đỏ, hồng đậm, cúc đỏ, các loại lá",
    },

    {
        mahoa: 20,
        maloai: 4,
        tenhoa: "Nắng nhẹ nhàng",
        dongia: 50000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa cúc xanh, hoa lys vàng, lá đệm",
    },

    {
        mahoa: 21,
        maloai: 4,
        tenhoa: "Thanh tao",
        dongia: 120000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa thủy tiên, cúa trắng, cúc dại tím, các loại lá đệm",
    },

    {
        mahoa: 22,
        maloai: 4,
        tenhoa: "Tinh khiết",
        dongia: 110000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hồng trắng và salem",
    },

    {
        mahoa: 23,
        maloai: 4,
        tenhoa: "Mùa xuân chín",
        dongia: 150000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hồmg cam, cúc xanh, thuỷ tiên và các loại lá",
    },

    {
        mahoa: 24,
        maloai: 4,
        tenhoa: "Rực rở nắng vàng",
        dongia: 75000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hồng vàng và cúc vàng",
    },

    {
        mahoa: 25,
        maloai: 3,
        tenhoa: "Love Candy",
        dongia: 95000,
        hinh: "https://picsum.photos/150/150/?random=",
        mota: "Hoa hồng trắng tinh khôi xen với hoa hồng màu hồng nhạt, điểm thêm baby trắng và lá măng",
    },
];

export function getAllHoa() {
    return listHoa;
}

export function getListHoa(maLoai) {
    return listHoa.filter((hoa) => hoa.maloai === maLoai);
}

export function getHoa(maHoa) {
    return listHoa.find((hoa) => hoa.mahoa === maHoa);
}
